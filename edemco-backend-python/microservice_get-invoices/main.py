...
def run_get_invoices():
    ...
    * Ejecuta un script Python en un subproceso y espera su finalización.
    """
    process = subprocess.Popen(["C:/edemco--pruebas/edemco-backend-python/venv/Scripts/python", "microservice_get-invoices/get_invoices.py"])
    try:
        process.wait()
...
def run_obser():
    ...
    * Ejecuta un script Python en un subproceso y espera su finalización.
    """
    process = subprocess.Popen(["C:/edemco--pruebas/edemco-backend-python/venv/Scripts/python", "microservice_get-invoices/cufe.py"])
    try:
        process.wait()
    except KeyboardInterrupt:
        process.terminate()
